...
let test_rev_table _ = 
for i = 0 to 63 do
ignore (Hexa.hexa_rev_map.(i))
done
...
let test_local _ =
 ...
 Loc.load_resource_blocking "en";
 let r =Loc.get_res' "en" 0 in
 assert_equal "aaaa" r.text;
 assert_raises ResourceNotAvailable (fun () -> Loc.get_res' "en" 99);
 let r =Loc.get_res' "cz" 0 in
 assert_equal "aaaa" r.text;
 Loc.load_resource_blocking "cz";
 let r =Loc.get_res' "cz" 2 in
 assert_equal "3333" r.text;
 try
   Loc.load_resource_blocking "xx";
